...
using GFW.Sharp.Lite.Proxy.GFWPress;
using System;
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (arguments.Has("-server"))
            {
                ...
                var server = new GFWPressListener(IPAddress.Parse(lip), lport, IPAddress.Parse(rip), rport, key, GFWPressWorkMode.Server);
                server.Start();
            }
            else if (arguments.Has("-client"))
            {
                ...
                Logger.Write(string.Format("remote port:\t{0}", rport));
                SecretKey key = new Encrypt().getPasswordKey(p);
                var client = new GFWPressListener(IPAddress.Parse(lip), lport, IPAddress.Parse(rip), rport, key, GFWPressWorkMode.Client);
                client.Start();
            }
            else
            {
        ...
        private static void ShowHelp()
        {
            ...
            System.Console.WriteLine("-lip\t\t\tlocal listen ip, default is 0.0.0.0.");
