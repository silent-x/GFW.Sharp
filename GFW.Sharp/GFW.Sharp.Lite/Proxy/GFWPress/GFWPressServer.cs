...
namespace GFW.Sharp.Lite.Proxy.GFWPress
{
    ...
    public class GFWPressServer : Client
    {
        ...
        public GFWPressServer(Socket ClientSocket, DestroyDelegate Destroyer, IPEndPoint MapTo, SecretKey key) : base(ClientSocket, Destroyer)
        {
            this.MapTo = MapTo;
            _key = key;
        ...
        protected override void OnClientReceive(IAsyncResult ar)
        {
            try
            {
                ...
                System.Array.Copy(Buffer, 0, receivedBytes, 0, receivedBytes.Length);
                ...
                DestinationSocket.BeginSend(decrypt_bytes, 0, decrypt_bytes.Length, SocketFlags.None, new AsyncCallback(this.OnRemoteSent), DestinationSocket);
            }
            catch
            {
                Dispose();
        ...
        protected override void OnRemoteReceive(IAsyncResult ar)
        {
            try
            {
                ...
                byte[] receivedBytes = new byte[Ret];
                System.Array.Copy(RemoteBuffer, 0, receivedBytes, 0, Ret);
                Encrypt aes = new Encrypt();
                byte[] sendingBytes = aes.encryptNet(_key, receivedBytes);
                ClientSocket.BeginSend(sendingBytes, 0, sendingBytes.Length, SocketFlags.None, new AsyncCallback(this.OnClientSent), ClientSocket);
            }
            catch
            {
                Dispose();
